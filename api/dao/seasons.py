...
def get_by_year_and_level(year: str, level_name: str):
    ...
    raise LookupError(f"No team not found for level ({level_name}) and year ({year})")
...
def update(season: SeasonUpdate):
    ...
    try:
        with db() as session:
            session.execute(stmt)
            stmt = text(
                """SELECT * FROM mhac.season_teams WHERE season_id = :season_id"""
            )
...
def create(season: SeasonNew, level: Level):
    ...
    stmt = text(
        """INSERT INTO mhac.seasons(id, name, year, level_id, sport_id, start_date, roster_submission_deadline, tournament_start_date, archive, slug)
                VALUES
    ...
    try:
        with db() as session:
            ...
            for team in season.season_teams:
                add_team_to_season(
                    season_id=new_season_id, team_id=team.team_id, session=session
                )
