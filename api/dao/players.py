...
def get_list() -> List[PlayerCreate]:
    player_list = []
    stmt = text(
        """SELECT person.* FROM mhac.person INNER JOIN mhac.person_type ON person.person_type = person_type.id WHERE person_type.type = 'Player' """
    )
    with db() as DB:
        result = DB.execute(stmt)
        for row in result:
...
def get_team_list(slug):
    player_list = []
    stmt = text(
        """SELECT * FROM mhac.person INNER JOIN mhac.person_type ON person.person_type = person_type.id WHERE person_type.type = 'Player' and slug = :slug"""
    )
...
def update(id, Player: PlayerCreate):
    ...
    with db() as DB:
        result = DB.execute(stmt)
    return f'Successfully updated player {Player.first_name} {Player.last_name}'
...
def create_player(player: PlayerCreate):
    ...
    stmt = text(
        """INSERT INTO mhac.person (id, first_name, last_name, birth_date, height, number, position, person_type, team_id) VALUES (:id, :first_name, :last_name, :birth_date, :height, :number, :position, :person_type, :team_id) """
    )
    stmt = stmt.bindparams(
        id=uuid4(),
        first_name=player.first_name,
    ...
    return 'Succesfully Created Player'