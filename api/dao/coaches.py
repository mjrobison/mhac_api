...
def create_coach(coach: Coach):
    ...
    print(uuid4())
    stmt = text(
        """INSERT INTO mhac.person (id, first_name, last_name, person_type, team_id) VALUES (:id, :first_name, :last_name, :person_type, :team_id) """
    )
...
def get_coach_list(person_type) -> List[CoachOut]:
    ...
    print(person_type)
    stmt = text(
        """SELECT person.id, person.first_name, person.last_name, person_type.type, person.team_id FROM mhac.person INNER JOIN mhac.person_type ON person.person_type = person_type.id WHERE person_type.type = :person_type """
    )
